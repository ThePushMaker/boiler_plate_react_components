...
const Navbar = () => {
  const liStyles = 'list-none py-0 px-[20px] relative lg:mb-0 mb-[25px]'
  ...
  return (
   <>
      <nav className='w-screen flex items-center justify-between bg-[#1b2430] py-[20px] px-[80px] shadow-sm
       '>
        <a href='index.html'>
        ...
        <div>
          <ul id='navbar' className={`${isClicked ? '#navbar active' : '#navbar'} flex flex-col justify-start content-start
          fixed top-[90px] right-[-300px] w-[300px] h-[100vh] bg-[#2a3239] shadow-2xl pt-[40px] pl-[10px] ease-in-out duration-300
        ...
        <div id='mobile' className='lg:hidden block' onClick={handleClick} >
          <i id='bar' className={`${isClicked ? 'fas fa-times' : 'fas fa-bars'} text-[24px] cursor-pointer`} ></i>
        </div>
      </nav>
